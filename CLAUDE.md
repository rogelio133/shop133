...
**Since `3.7` there is a remedy that does not depend on Windows changing its mind: run the `.dll` with `dotnet`, not the `.exe`.** What Smart App Control rejects is the SDK-generated **apphost** — an unsigned `.exe` with no reputation — so `dotnet tests\...\Foo.Tests.dll` sidesteps the evaluation because `dotnet.exe` is Microsoft-signed. In `3.7` **neither documented remedy worked**: `Inventory.Tests.exe` stayed blocked across two retries *and* a `-c Release` rebuild, while `Payments.Tests.exe`, created half an hour later, ran first try — confirming the block is per file, not per project. Try the `.dll` first now; the two paragraphs below are the history and still describe the symptom.
...
**The `ConnectionStrings:RabbitMq` `UseSetting` stopped being decorative.** In `3.1` the key merely had to exist — nothing published, and a bus without a broker just warns and retries, so the suite passed with RabbitMQ stopped. Now `Publish` on the RabbitMQ transport **waits for a connection instead of failing fast**: with the broker down the POST hangs rather than erroring, which made `docker compose up -d` a prerequisite of `Orders.Tests`. **That prerequisite is gone since `3.7`**, which swaps the bus for the in-memory harness — but the underlying fact about `Publish` is unchanged and still bites anything that talks to the real transport.
...
**`Payments.Tests` exists since `3.7`** (9 tests) and covers all of the above, which `3.5` had verified by hand. **`Payments.API` still has no `public partial class Program { }`** and does not need one: the suite builds its own `ServiceCollection` around `StockReservedConsumer` instead of booting the service.

**`Inventory.Tests` exists since `3.7`** (9 tests), automating what `3.4` had verified by hand, and **`Inventory.API` still has no `public partial class Program { }`** — same reasoning as Payments. That is also where the `SqlServerContainerFixture` extraction finally got decided. Also still missing, deliberately: **optimistic concurrency on `StockItem`** — two `OrderCreated` for the same product processed concurrently both read the same `QuantityAvailable` and both pass the check, so they can over-reserve. Real gap, no owner yet.
...
│   ├── Shop133.TestUtilities/       SqlServerContainerFixture, shared by the four suites (3.7)
│   └── Services/
...
Already in use: `xunit.v3` 4.0.0, `NetArchTest.Rules` 1.3.2, since `1.7` `Microsoft.AspNetCore.Mvc.Testing` 10.0.11 + `Testcontainers.MsSql` 4.14.0, and since `3.7` `Microsoft.Data.SqlClient` 6.1.1 + `xunit.v3.extensibility.core` 4.0.0 (both in `Shop133.TestUtilities` only — that project references no service, so what used to arrive transitively has to be declared). `WireMock.Net` was deleted in `3.3` with the synchronous debt.

**`MassTransit.TestFramework` was never added and is not needed** — `3.7` checked before installing: `AddMassTransitTestHarness` and `ITestHarness` live in the plain **`MassTransit`** package (`MassTransit.DependencyInjectionTestingExtensions`, namespace `MassTransit.Testing`), and reach the test projects transitively via `<Svc>.API → MassTransit.RabbitMQ`. That also retires the warning this file used to carry: there is **no** MassTransit `PackageReference` under `tests/` whose pin `MassTransitPackages_StayOnMajorVersion8` (which only scans `src/`) would fail to police. Arriving transitively, the harness version is by construction the one the service uses.

Still subject to "ask before adding a NuGet package": `Testcontainers.RabbitMq` (rejected in `2.4` and again in `3.7`) and `Respawn` — **considered and rejected twice**, in `1.7` and again in `2.4`, see [docs/fase_1_7.md](docs/fase_1_7.md) and [docs/fase_2_4.md](docs/fase_2_4.md).

**`tests/Shop133.TestUtilities` exists since `3.7`** and holds exactly one file: `SqlServerContainerFixture`, which had been copied byte-for-byte into `Catalog.Tests` and `Orders.Tests`. `2.4` deferred the extraction ("two occurrences are not a pattern") and named `3.7` as the moment to decide with four copies in hand; `diff -u` then showed the two existing ones differed **only in the namespace line and the comment prose**. The containment rule is written in its `.csproj`: **only what all four suites use identically goes in.** The factories/hosts stay in their own projects — they genuinely diverge, and sharing what diverges is how a utilities project becomes a junk drawer.

**Consumer suites do not use `WebApplicationFactory`.** `Inventory.API` and `Payments.API` have no HTTP surface, so `Inventory.Tests`/`Payments.Tests` build a `ServiceCollection` with the consumer, the real `DbContext` and `AddMassTransitTestHarness`. That is why neither service got a `public partial class Program { }` — the question `3.4` and `3.5` left open, answered **no**. The price is explicit: **nothing verifies that their `Program.cs` actually registers the consumer**; that gap belongs to `8.2`.

**Three harness traps, all measured in `3.7` and all of which produce green tests that prove nothing.** (1) `harness.InactivityTask` is **one task that completes the first time** the bus goes idle — every later `await` returns instantly, so wait **once per test, after all publishes**, never inside a publish helper. (2) **`harness.Consumed` is indexed by `MessageId`**: two deliveries of the same id collapse into one entry and a message with no id is not recorded at all — precisely the cases an idempotency test needs, so never count it to wait. (3) An idempotency test needs **`Assert.Empty(Published<Fault<T>>())`** or it passes with the guard deleted: without the guard the duplicate does not get skipped, it dies on the duplicate-key `INSERT` into `ProcessedMessages` and so publishes nothing either way. Counting business events cannot tell *skipped silently* from *blew up*. Verified by breaking both consumers on purpose.

**The in-memory transport delivers concurrently, so `cfg.ConcurrentMessageLimit = 1` is set in both consumer hosts.** Without it, two deliveries of the same order can be processed at once, neither sees the other's row, both `INSERT` and one dies on the duplicate key — the same test gave 2 events on one run and 1 on the next. **That is not a test defect: it is the concurrency hole `3.6` recorded with no owner**, reproduced by accident. Pinning to 1 models a *redelivery*, which is sequential by definition; the hole stays open and documented, not papered over.
...
# What works today. Each test project is its own executable (rule 5b) — but launch
# it through `dotnet <dll>`, not the .exe: Smart App Control blocks the SDK's
# unsigned apphost and, unlike in 1.7 and 3.5, in 3.7 neither retrying nor a
# Release build cleared it. `dotnet.exe` is Microsoft-signed, so loading the dll
